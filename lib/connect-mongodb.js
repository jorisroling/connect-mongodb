...
var MONGOSTORE = module.exports = function MongoStore(options, callback) {
  ...
  function getCollection(db, callback) {
    db.collection(options.collection || _defaults.collection, function (err, col) {
      if (err) callback(err);
      _collection = col;
		console.log('connect-mongodb Collection set: '+(options.collection || _defaults.collection));
      callback();
    });
  }
  ...
  if (options.reapInterval !== -1) {
    var reap_interval = setInterval(function () {
		if (_collection) _collection.remove({expires: {'$lte': Date.now()}}, function () { });
...
MONGOSTORE.prototype.get = function (sid, cb) {
  cb = _default(cb);
  if (_collection) _collection.findOne({_id: sid}, function (err, data) {
    try {
      if (data) {
...
MONGOSTORE.prototype.set = function (sid, sess, cb) {
  ...
  if (_collection) _collection.update({_id: sid}, update, {upsert: true}, function (err, data) {
...
MONGOSTORE.prototype.destroy = function (sid, cb) {
  if (_collection) _collection.remove({_id: sid}, _default(cb));
};
...
MONGOSTORE.prototype.length = function (cb) {
  if (_collection) _collection.count({}, _default(cb));
};
...
MONGOSTORE.prototype.clear = function (cb) {
  if (_collection) _collection.drop(_default(cb));
};
